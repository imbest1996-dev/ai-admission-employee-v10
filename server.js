...
const VERSION='V10.1.1', STARTED_AT=Date.now();
...
app.post('/api/public/demo',(req,res)=>{const message=String(req.body?.message||'').trim();if(!message)return res.status(400).json({error:'message required'});const sessionId=String(req.body?.session_id||'').trim().slice(0,100);const setup={courses:'NEET Dropper — sample fee ₹45,000\nJEE Dropper — sample fee ₹40,000',demo:'Free counselling available Mon–Sat'};let out=fallback(message,setup);if(sessionId){const demoInstituteId=0;out=applyConversationContext(message,out,loadConversation(demoInstituteId,sessionId,setup));saveConversation(demoInstituteId,sessionId,out,message);}event('demo_message');res.json(out)});
