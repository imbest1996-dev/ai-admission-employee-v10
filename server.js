...
const VERSION='V10.0', STARTED_AT=Date.now();
...
app.post('/api/public/event',(req,res)=>{event(req.body?.event_type||'page_view',null,req.body?.meta||{});res.json({ok:true})});
...
app.get('/api/me',auth,(req,res)=>{const x=db.prepare('SELECT id,name,email,location,courses,demo,plan,subscription_status,trial_ends_at FROM institutes WHERE id=?').get(req.instituteId);res.json({...x,entitled:entitlement(req.instituteId)})});
app.patch('/api/me',auth,(req,res)=>{const old=db.prepare('SELECT * FROM institutes WHERE id=?').get(req.instituteId);const {name=old.name,location=old.location,courses=old.courses,demo=old.demo}=req.body||{};db.prepare('UPDATE institutes SET name=?,location=?,courses=?,demo=? WHERE id=?').run(name,location,courses,demo,req.instituteId);res.json({ok:true})});
...
const l=out.lead;const r=db.prepare('INSERT INTO leads(institute_id,student_message,course,intent,temperature,next_action,summary) VALUES(?,?,?,?,?,?,?)').run(req.instituteId,message,l.course,l.intent,l.temperature,l.next_action,l.summary);out.lead.id=Number(r.lastInsertRowid);res.json(out)});
...
app.get('/api/funnel',auth,(req,res)=>res.json(db.prepare('SELECT event_type,COUNT(*) n FROM funnel_events GROUP BY event_type ORDER BY n DESC').all()));
